            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">Formula Item</strong><br>
                        <a href="{{ route('formula-utama.index') }}" class="{{ $is_trash ? 'text-success':'text-muted' }}"> All
                            <span class="badge badge-pill badge-info">{{ $formula_utama_count }}</span>
                        </a>
                        &nbsp; | &nbsp;
                        <a href="{{ route('formula-utama.index') }}?status=trash" class="{{ $is_trash ? 'text-muted':'text-danger' }}">
                            Trash
                            <span class="badge badge-pill badge-danger">{{ $trash_count }}</span>
                        </a>
                        <a href="{{route('formula-utama.create')}}" class="btn btn-sm btn-primary pull-right">Tambah Data</a>
                    </div>
                    <div class="card-body">
                        @if( session('msg') )
                            <?php $msg = session('msg'); ?>
                            ...
                            <tbody>
                             @if($records->count()>0)
                                ...
                                <tr>
                                    ...
                                    <td >{{ $row->nama}}</td>
                                    <td >Rp. {{number_format($row->harga_formula_utama, 0)}}</td>
                                    <td >
                                        @if($row->id != 0)
                                            @if( $row->deleted_at == null )
                                                <a href="{{ route('formula-utama.edit',$row->id) }}" class="btn btn-success btn-sm">Edit</a>
    ...
    <script type="text/javascript">
        $(document).ready(function(){
         ...
         function execRemove(method, id) {
             $("#action-form").attr('action', 'formula-utama/delete/' + id);
         ...
         function restore (id) {
             $("#action-form").attr('action', 'formula-utama/restore/' + id);
