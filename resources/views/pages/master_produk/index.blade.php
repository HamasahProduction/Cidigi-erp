            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        <a href="{{ route('master-produk.index') }}" class="{{ $is_trash ? 'text-success':'text-muted' }}"> All
                            <span class="badge badge-pill badge-info">{{ $produk_count }}</span>
                        </a>
                        &nbsp; | &nbsp;
                        <a href="{{ route('master-produk.index') }}?status=trash" class="{{ $is_trash ? 'text-muted':'text-danger' }}">
                            Trash
                            <span class="badge badge-pill badge-danger">{{ $trash_count }}</span>
                        </a>
                        <a href="{{route('master-produk.create')}}" class="btn btn-sm btn-primary pull-right">Tambah Data</a>
                    </div>
                    <div class="card-body">
                        @if( session('msg') )
                            <?php $msg = session('msg'); ?>
                                    ...
                                    <td >
                                        @if($row->id != 0)
                                            @if( $row->deleted_at == null )
                                                <a href="{{ route('master-produk.edit',$row->id) }}" class="btn btn-success btn-sm">Edit</a>
    ...
    <script type="text/javascript">
       $(document).ready(function(){
        ...
        function execRemove(method, id) {
            $("#action-form").attr('action', 'master-produk/delete/' + id);
        ...
        function restore (id) {
            $("#action-form").attr('action', 'master-produk/restore/' + id);
